...
function renderPrediction(p) {
  ...
  const eloParts = [];
  if (p.elo_home) eloParts.push(`${p.home} (${p.elo_home})`);
  if (p.elo_away) eloParts.push(`${p.away} (${p.elo_away})`);
  const eloLine = eloParts.length
    ? `<div class="card-row"><span class="label">Élo</span><span class="value">${eloParts.join(" · ")}</span></div>`
    : "";
