...
CLUBELO_TIMEOUT_SECONDS = 6
CLUBELO_FAILURE_THRESHOLD = 3
CLUBELO_COOLDOWN_SECONDS = 180
_clubelo_breaker = {"failures": 0, "skip_until": 0.0}
_elo_history_cache = {}


def _clubelo_breaker_is_open() -> bool:
    return time.time() < _clubelo_breaker["skip_until"]


def _clubelo_breaker_record_failure():
    _clubelo_breaker["failures"] += 1
    if _clubelo_breaker["failures"] >= CLUBELO_FAILURE_THRESHOLD:
        _clubelo_breaker["skip_until"] = time.time() + CLUBELO_COOLDOWN_SECONDS
        logger.warning(
            f"ClubElo : {_clubelo_breaker['failures']} échecs consécutifs, "
            f"pause de {CLUBELO_COOLDOWN_SECONDS}s avant nouvel essai."
        )


def _clubelo_breaker_record_success():
    _clubelo_breaker["failures"] = 0
    _clubelo_breaker["skip_until"] = 0.0


def _clubelo_get(name: str):
    """Essaie HTTPS puis HTTP, valide que la réponse ressemble à un vrai CSV
    ClubElo (le site renvoie parfois une page d'erreur/surcharge avec un
    statut 200), et respecte le coupe-circuit pour ne pas re-taper un
    service en panne à chaque requête."""
    if _clubelo_breaker_is_open():
        return None
    for scheme in ("https", "http"):
        url = f"{scheme}://api.clubelo.com/{requests.utils.quote(name, safe='')}"
        try:
            resp = requests.get(url, timeout=CLUBELO_TIMEOUT_SECONDS, headers={"User-Agent": "MatchGuard/1.0"})
            resp.raise_for_status()
            body = resp.text.strip()
            if not body or "\n" not in body:
                continue
            header = body.splitlines()[0].lower()
            if "elo" not in header or "from" not in header:
                logger.warning(f"Réponse ClubElo inattendue pour '{name}': {body[:120]!r}")
                continue
            _clubelo_breaker_record_success()
            return body
        except requests.RequestException:
            continue
    _clubelo_breaker_record_failure()
    return None
...
def get_team_elo_history(team_name: str, days: int = 365) -> list:
    cache_key = (team_name.strip().lower(), days)
    if cache_key in _elo_history_cache:
        return _elo_history_cache[cache_key]
    ...
    _elo_history_cache[cache_key] = result
    return result
