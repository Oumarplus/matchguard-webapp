...
def _elo_background_updater():
    ...
    job_queue du bot Telegram, donc ce thread en tient lieu — démarré au
    premier before_request (voir _kick_off_elo_thread), pas à l'import du
    module, pour survivre correctement à un éventuel fork Gunicorn."""
    while True:
        ok = True
        try:
            seed_homemade_elo()
        ...
        # Nouvel essai rapide (1 min) après un échec, sinon rythme normal (6h)
        time.sleep(60 if not ok else 6 * 3600)


_elo_thread_lock = threading.Lock()
_elo_thread_started = False


def _ensure_elo_thread_started():
    """Démarre le thread de mise à jour Elo une seule fois, mais au moment
    de la première vraie requête HTTP plutôt qu'à l'import du module :
    si Gunicorn utilise le préchargement (--preload) puis forke des workers,
    un thread démarré à l'import ne survit PAS dans les processus enfants
    forkés, qui sont pourtant ceux qui traitent réellement les requêtes.
    Démarrer au premier before_request garantit qu'on est déjà dans le bon
    processus (post-fork)."""
    global _elo_thread_started
    if _elo_thread_started:
        return
    with _elo_thread_lock:
        if _elo_thread_started:
            return
        threading.Thread(target=_elo_background_updater, daemon=True).start()
        _elo_thread_started = True


@app.before_request
def _kick_off_elo_thread():
    _ensure_elo_thread_started()
...
@app.route("/api/elostatus")
def api_elostatus():
    state = load_elo_state()
    return jsonify({
        "thread_started": _elo_thread_started,
        "seeded": state.get("seeded", False),
        "teams_known": len(state.get("ratings", {})),
        "competitions_updated": state.get("seen_competitions", []),
        "sample_teams": list(state.get("ratings", {}).keys())[:10],
    })
