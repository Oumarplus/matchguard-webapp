...
def implied_probabilities_totals(event: dict, target_point: float = 2.5) -> dict | None:
    ...
    if not over_probs:
        return None
    return {"over": round(statistics.mean(over_probs), 3), "under": round(statistics.mean(under_probs), 3)}
...
def api_predict(code):
    ...
    if code not in COMPETITIONS:
        return jsonify({"error": "Code inconnu"}), 404
    try:
        return jsonify({"competition": COMPETITIONS[code], "predictions": get_predictions(code)})
    except requests.HTTPError as e:
        return jsonify({"error": str(e)}), 502
